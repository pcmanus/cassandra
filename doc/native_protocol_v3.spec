...
7. User defined types
  ...
  A UDT value is composed of successive [bytes] values, one foe each field of the UDT
  value (in the order defined by the type). A UDT value will generally have one value
  for each field of the type it represent, but it is allowed to have less values than
  the type has fields.
