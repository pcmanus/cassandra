...
|@max_compaction_threshold@   |32                          |Maximum number of SSTables processed by one minor compaction.|
...
The @COUNT@ keyword can be used with parenthesis to enclose the list of column names (or @*@). If so, the query will return a single result: the number of rows matching the query. Note that @COUNT(1)@ is supported as an alias for @COUNT(*)@.
...
The @ORDER BY@ option allows to select the order of the returned results. It takes a column on which to order the result as argument. Currently only the first of the clustering keys is allowed as argument to @ORDER BY@.
...
Timestamp types can also be input as string literals in any of the following ISO 8601 formats, each representing the time and date Mar 2, 2011, at 04:05:00 AM, GMT.:
