...
public class SchemaLoader
{
    ...
    public static CFMetaData perRowIndexedCFMD(String ksName, String cfName)
    {
        ...
        ByteBuffer kName = ByteBufferUtil.bytes("key");

        ByteBuffer cName = ByteBufferUtil.bytes("indexed");

        cfm.addOrReplaceColumnDefinition(ColumnDefinition.partitionKeyDef(cfm, kName, AsciiType.instance, null));
    ...
    public static void insertData(String keyspace, String columnFamily, int offset, int numberOfRows)
    {
        ...
        {
            RowUpdateBuilder builder = new RowUpdateBuilder(cfm, FBUtilities.timestampMicros(), "key"+i);
            ColumnDefinition cdef = ColumnDefinition.compactValueDef(cfm, ByteBufferUtil.bytes("col" + i), cfm.getDefaultValidator());

            builder.add(cdef, ByteBufferUtil.bytes("val" + i));
