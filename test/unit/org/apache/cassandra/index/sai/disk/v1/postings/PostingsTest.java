...
public class PostingsTest extends SaiRandomizedTest
{
    ...
    public void testSingleBlockPostingList() throws Exception
    {
        ...
        try (PostingsWriter writer = new PostingsWriter(group, blockSize))
        {
            postingPointer = writer.write(expectedPostingList);
            writer.complete();
        }

        IndexInput input = group.get(IndexComponent.POSTING_LISTS).openInput();
        ...
        input = group.get(IndexComponent.POSTING_LISTS).openInput();
    ...
    public void testMultiBlockPostingList() throws Exception
    {
        ...
        IndexComponentInfo.Reader postingLists = group.get(IndexComponent.POSTING_LISTS);
    ...
    {
        ComponentGroup.Writer group = indexDescriptor.newPerIndexGroupWriter(indexContext);
        try (PostingsWriter writer = new PostingsWriter(group))
        {
    ...
    private void testAdvance(IndexComponentInfo.Reader postingLists, long fp, ArrayPostingList expected, int[] targetIDs) throws IOException
    {
        expected.reset();
        final CountingPostingListEventListener listener = new CountingPostingListEventListener();
    ...
    private PostingsReader openReader(IndexComponentInfo.Reader postingLists, long fp, QueryEventListener.PostingListEventListener listener) throws IOException
    {
        IndexInput input = postingLists.openInput();
        input.seek(fp);
