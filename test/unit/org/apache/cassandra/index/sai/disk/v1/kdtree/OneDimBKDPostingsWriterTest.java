...
public class OneDimBKDPostingsWriterTest extends SaiRandomizedTest
{
    ...
    public void shouldWritePostingsForEligibleNodes() throws IOException
    {
        ...
        OneDimBKDPostingsWriter writer = new OneDimBKDPostingsWriter(leaves, new IndexWriterConfig("test", 2, 1), group::logMessage);
        ...
        long fp;
        try (IndexOutputWriter output = group.addOrGet(IndexComponent.KD_TREE_POSTING_LISTS).openOutput())
        {
            fp = writer.finish(output);
        }

        IndexComponentInfo.Reader kdTreePostings = group.get(IndexComponent.KD_TREE_POSTING_LISTS);
    ...
    public void shouldSkipPostingListWhenSamplingMisses() throws IOException
    {
        ...
        ComponentGroup.Writer group = indexDescriptor.newPerIndexGroupWriter(indexContext);
        OneDimBKDPostingsWriter writer = new OneDimBKDPostingsWriter(leaves, new IndexWriterConfig("test", 5, 1), group::logMessage);
        ...
        BKDPostingsIndex postingsIndex = new BKDPostingsIndex(group.get(IndexComponent.KD_TREE_POSTING_LISTS).createFileHandle(), fp);
    ...
    public void shouldSkipPostingListWhenTooFewLeaves() throws IOException
    {
        List<PackedLongValues> leaves = Collections.singletonList(postings(1, 2, 3));
        ComponentGroup.Writer group = indexDescriptor.newPerIndexGroupWriter(indexContext);
        OneDimBKDPostingsWriter writer = new OneDimBKDPostingsWriter(leaves, new IndexWriterConfig("test", 2, 2), group::logMessage);
    ...
    private void assertPostingReaderEquals(IndexComponentInfo.Reader kdTreePostingLists, BKDPostingsIndex postingsIndex, int nodeID, int[] postings) throws IOException
    {
