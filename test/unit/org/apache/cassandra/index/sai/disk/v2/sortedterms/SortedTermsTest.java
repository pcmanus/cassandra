...
public class SortedTermsTest extends SaiRandomizedTest
{
    ...
    public void testLexicographicException() throws Exception
    {
        ...
        try (MetadataWriter metadataWriter = new MetadataWriter(group))
        {
            NumericValuesWriter blockFPWriter = new NumericValuesWriter(group.addOrGet(IndexComponent.PRIMARY_KEY_BLOCK_OFFSETS),
                                                                        metadataWriter, true);
            try (SortedTermsWriter writer = new SortedTermsWriter(group.addOrGet(IndexComponent.PRIMARY_KEY_BLOCKS),
                                                                  metadataWriter,
                                                                  blockFPWriter,
    ...
    public void testFileValidation() throws Exception
    {
        ...
        assertTrue(validateComponent(group, IndexComponent.PRIMARY_KEY_TRIE, true));
        assertTrue(validateComponent(group, IndexComponent.PRIMARY_KEY_TRIE, false));
        assertTrue(validateComponent(group, IndexComponent.PRIMARY_KEY_BLOCKS, true));
        assertTrue(validateComponent(group, IndexComponent.PRIMARY_KEY_BLOCKS, false));
        assertTrue(validateComponent(group, IndexComponent.PRIMARY_KEY_BLOCK_OFFSETS, true));
        assertTrue(validateComponent(group, IndexComponent.PRIMARY_KEY_BLOCK_OFFSETS, false));
    }
                                       ...
                                       ThrowingConsumer<SortedTermsReader.Cursor> testCode) throws IOException
    {
        ...
        try (FileHandle trieHandle = group.get(IndexComponent.PRIMARY_KEY_TRIE).createFileHandle();
    ...
    private boolean validateComponent(ComponentGroup.Reader group, IndexComponent indexComponent, boolean checksum)
    {
        try (IndexInput input = group.get(indexComponent).openInput())
        {
            if (checksum)
