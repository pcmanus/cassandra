...
public class DirectoriesTest
{
    ...
    private static void createTestFiles() throws IOException
    {
        for (TableMetadata cfm : CFM)
        {
            ...
            files.put(cfm.table, fs);
            File dir = cfDir(cfm);
            dir.mkdirs();

            createFakeSSTable(dir, cfm.table, 1, fs);
            createFakeSSTable(dir, cfm.table, 2, fs);

            File backupDir = new File(dir, Directories.BACKUPS_SUBDIR);
            backupDir.mkdir();
            createFakeSSTable(backupDir, cfm.table, 1, fs);

            File snapshotDir = new File(dir, Directories.SNAPSHOT_SUBDIR + File.separator + "42");
            snapshotDir.mkdirs();
            createFakeSSTable(snapshotDir, cfm.table, 1, fs);
        }
    }
    ...
    public void testStandardDirs()
    {
        for (TableMetadata cfm : CFM)
        {
            ...
            Descriptor desc = new Descriptor(cfDir(cfm), KS, cfm.table, 1, SSTableFormat.Type.BIG);
    ...
    public void testSecondaryIndexDirectories()
    {
        ...
        Descriptor parentDesc = new Descriptor(parentDirectories.getDirectoryForNewSSTables(), KS, PARENT_CFM.table, 0, SSTableFormat.Type.BIG);
        Descriptor indexDesc = new Descriptor(indexDirectories.getDirectoryForNewSSTables(), KS, INDEX_CFM.table, 0, SSTableFormat.Type.BIG);
        ...
        Descriptor parentSnapshot = new Descriptor(parentSnapshotDirectory, KS, PARENT_CFM.table, 0, SSTableFormat.Type.BIG);
        createFile(parentSnapshot.filenameFor(Component.DATA), 30);
        Descriptor indexSnapshot = new Descriptor(indexSnapshotDirectory, KS, INDEX_CFM.table, 0, SSTableFormat.Type.BIG);
    ...
    private void checkFiles(TableMetadata cfm, Directories directories)
    {
        ...
        for (File f : files.get(cfm.table))
        {
