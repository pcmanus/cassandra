...
public class MigrationManagerTest
{
    ...
    public void addNewTable() throws ConfigurationException
    {
        ...
        assertFalse(Schema.instance.getKeyspaceMetadata(ksName).tables.get(cfm.table).isPresent());
        MigrationManager.announceNewTable(cfm);

        assertTrue(Schema.instance.getKeyspaceMetadata(ksName).tables.get(cfm.table).isPresent());
        assertEquals(cfm, Schema.instance.getKeyspaceMetadata(ksName).tables.get(cfm.table).get());
    ...
    public void dropCf() throws ConfigurationException
    {
        ...
        ColumnFamilyStore store = Keyspace.open(cfm.keyspace).getColumnFamilyStore(cfm.table);
        assertNotNull(store);
        store.forceBlockingFlush();
        assertTrue(store.getDirectories().sstableLister(Directories.OnTxnErr.THROW).list().size() > 0);

        MigrationManager.announceTableDrop(ks.name, cfm.table);

        assertFalse(Schema.instance.getKeyspaceMetadata(ks.name).tables.get(cfm.table).isPresent());
    ...
    public void dropKS() throws ConfigurationException
    {
        ...
        ColumnFamilyStore cfs = Keyspace.open(cfm.keyspace).getColumnFamilyStore(cfm.table);
        assertNotNull(cfs);
        cfs.forceBlockingFlush();
    ...
    public void createEmptyKsAddNewCf() throws ConfigurationException
    {
        ...
        assertFalse(Schema.instance.getKeyspaceMetadata(newKs.name).tables.get(newCf.table).isPresent());

        //add the new CF to the empty space
        MigrationManager.announceNewTable(newCf);

        assertTrue(Schema.instance.getKeyspaceMetadata(newKs.name).tables.get(newCf.table).isPresent());
        assertEquals(Schema.instance.getKeyspaceMetadata(newKs.name).tables.get(newCf.table).get(), newCf);
        ...
        ColumnFamilyStore cfs = Keyspace.open(newKs.name).getColumnFamilyStore(newCf.table);
        assertNotNull(cfs);
        cfs.forceBlockingFlush();
