...
#            assert True, 'column did not exist'
    ...
    def test_counter_add(self):
        ...
        client.add(key, cf, updates[1], ConsistencyLevel.ONE);
        assert client.get_counter(key, path, ConsistencyLevel.ONE).value == values[0] + values[1]

        client.add(key, cf, updates[2], ConsistencyLevel.ONE);
        assert client.get_counter(key, path, ConsistencyLevel.ONE).value == values[0] + values[1] + values[2]
    ...
    def test_counter_replay(self):
        ...
        assert client.get_counter(key, path, ConsistencyLevel.ONE).value == 2 * v
    ...
    def test_counter_batch_operation(self):
        ...
        cf = 'Counter1'
        key = 'row4'
        counts = [ [ 234, 52345, -42 ], [ 12, -100, -10 ], [ 0, 5], [ -4 ], [ 42, 20000000023, 01 ] ]
        ...
        results = client.get_counter_slice(key, cf, SlicePredicate(column_names=totals.keys()), ConsistencyLevel.ONE)
        assert len(results) == len(totals)
        for c in results:
            assert c.value == totals[c.name]
