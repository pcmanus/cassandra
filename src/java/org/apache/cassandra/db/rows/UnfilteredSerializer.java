...
public class UnfilteredSerializer
{
    ...
    throws IOException
    {
        ...
        if (hasTimestamp)
        {
            long timestamp = hasTimestamp ? header.readTimestamp(in) : LivenessInfo.NO_TIMESTAMP;
    ...
    throws IOException
    {
        ...
        {
            skipComplexColumn(in, column, header, helper, hasComplexDeletion);
        }
    }

    public void skipRowBody(DataInputPlus in, SerializationHeader header, SerializationHelper helper, int flags) throws IOException
    {
        boolean isStatic = isStatic(flags);
        boolean hasTimestamp = (flags & HAS_TIMESTAMP) != 0;
        boolean hasTTL = (flags & HAS_TTL) != 0;
        ...
        if (header.useSparseColumnLayout(isStatic))
        {
            ...
            while ((i = in.readShort()) >= 0)
            {
                ...
                if (i < simpleCount)
                    ...
                    skipComplexColumn(in, columns.getComplex(i - simpleCount), header, helper, hasComplexDeletion);
            }
        }
        else
        {
            ...
            for (int i = 0; i < columns.complexColumnCount(); i++)
                skipComplexColumn(in, columns.getComplex(i), header, helper, hasComplexDeletion);
        }
    }
    ...
    private void skipComplexColumn(DataInputPlus in, ColumnDefinition column, SerializationHeader header, SerializationHelper helper, boolean hasComplexDeletion)
    throws IOException
    {
        if (hasComplexDeletion)
            header.skipDeletionTime(in);
