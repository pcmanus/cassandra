...
public class SinglePartitionReadCommand extends ReadCommand
{
    ...
    private UnfilteredRowIterator queryMemtableAndDiskInternal(ColumnFamilyStore cfs, boolean copyOnHeap)
    {
         ...
         *   1) We query all memtables and sstables simulateneously. This is our most generic strategy and the one we use
        ...
        if (clusteringIndexFilter() instanceof ClusteringIndexNamesFilter && queryNeitherCountersNorCollections())
            return queryMemtableAndSSTablesInTimestampOrder(cfs, copyOnHeap);

        return queryMemtableAndAllSSTables(cfs, copyOnHeap);
    }
    ...
    /**
     * Do the actual read by querying the memtable(s) and all relevant sstables simultaneously.
     */
    private UnfilteredRowIterator queryMemtableAndAllSSTables(ColumnFamilyStore cfs, boolean copyOnHeap)
    {
    ...
    /**
     * Do the actual read by querying the memtable(s) first, and then each relevant sstables sequentially by order of the sstable
     * max timestamp.
     *
     ...
     */
    private UnfilteredRowIterator queryMemtableAndSSTablesInTimestampOrder(ColumnFamilyStore cfs, boolean copyOnHeap)
    {
        Tracing.trace("Acquiring sstable references");
        ColumnFamilyStore.ViewFragment view = cfs.select(View.select(SSTableSet.LIVE, partitionKey()));
        ...
        ClusteringIndexNamesFilter filter = (ClusteringIndexNamesFilter)clusteringIndexFilter();
