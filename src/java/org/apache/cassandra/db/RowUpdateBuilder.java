...
public class RowUpdateBuilder
{
    ...
    public RowUpdateBuilder resetCollection(String columnName)
    {
        ...
        assert c.isStatic() || update.metadata().comparator.size() == 0 || regularBuilder != null : "Cannot set non static column " + c + " since no clustering hasn't been provided";
    ...
    {
        ColumnDefinition c = getDefinition(columnName);
        assert c.isStatic() || regularBuilder != null : "Cannot set non static column " + c + " since no clustering hasn't been provided";
