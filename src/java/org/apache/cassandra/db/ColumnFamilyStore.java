...
public class ColumnFamilyStore implements ColumnFamilyStoreMBean
{
    ...
    public void apply(PartitionUpdate update, SecondaryIndexManager.Updater indexer, OpOrder.Group opGroup, ReplayPosition replayPosition)
    {
        ...
        mt.put(update, indexer, opGroup, replayPosition);
        maybeUpdateRowCache(update.partitionKey());
    ...
    public boolean isFilterFullyCoveredBy(PartitionFilter filter, DataLimits limits, CachedPartition cached, int nowInSec)
    {
        ...
        // cells: if we were to check expired cells now, we could end up non-counting rows that were counted
        // when populating the cache, we might thus end up deciding the whole partition is cached when it's
        // really not (just some rows expired since the partition was cached). This is why we use
        // "rowsWithNonTombstoneCells".
        boolean wholePartitionCached = cached.rowsWithNonTombstoneCells() < metadata.getCaching().rowCache.rowsToCache;

        // Contrarily to the "wholePartitionCached" check above, we do want isFullyCoveredBy to take the
        // timestamp of the query into account when dealing with expired cells. Otherwise, we could think
        // the cached partition has enough live rows to satisfy the filter when it doesn't because some
        // are now expired.
        return wholePartitionCached || filter.isFullyCoveredBy(cached, limits, nowInSec);
    }
    ...
    public ClockAndCount getCachedCounter(ByteBuffer partitionKey, ClusteringPrefix clustering, ColumnDefinition column)
    {
    ...
    public void putCachedCounter(ByteBuffer partitionKey, ClusteringPrefix clustering, ColumnDefinition column, ClockAndCount clockAndCount)
    {
        if (CacheService.instance.counterCache.getCapacity() == 0L) // counter cache disabled.
            return;
