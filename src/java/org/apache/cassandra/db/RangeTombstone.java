...
public class RangeTombstone extends Interval<ByteBuffer, DeletionTime> implements OnDiskAtom
{
    ...
    {
        return serializedSize(Descriptor.Version.CURRENT.asMessagingVersion());
    }
    ...
    {
        return DBConstants.SHORT_SIZE + min.remaining()
             + 1 // serialization flag
             + DBConstants.SHORT_SIZE + max.remaining()
             + DeletionTime.serializer.serializedSize(data);
    }
