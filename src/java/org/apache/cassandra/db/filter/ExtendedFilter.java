...
public abstract class ExtendedFilter
{
    ...
    public static ExtendedFilter create(ColumnFamilyStore cfs, DataRange dataRange, List<IndexExpression> clause, int maxResults, boolean countCQL3Rows)
    {
        if (clause == null || clause.isEmpty())
            return new EmptyClauseFilter(cfs, dataRange, maxResults, countCQL3Rows);

        return new WithClauses(cfs, dataRange, clause, maxResults, countCQL3Rows);
    }

    protected ExtendedFilter(ColumnFamilyStore cfs, DataRange dataRange, int maxResults, boolean countCQL3Rows)
    {
        assert cfs != null;
    ...
    {
        return dataRange.getLiveCount(data);
    }
    ...
    public static class WithClauses extends ExtendedFilter
    {
        ...
        public WithClauses(ColumnFamilyStore cfs, DataRange range, List<IndexExpression> clause, int maxResults, boolean countCQL3Rows)
        {
            super(cfs, range, maxResults, countCQL3Rows);
            assert clause != null;
            this.clause = clause;
        ...
        public ColumnFamily prune(DecoratedKey rowKey, ColumnFamily data)
        {
            ...
            filter.collectReducedColumns(pruned, QueryFilter.gatherTombstones(pruned, iter), cfs.gcBefore());
            return pruned;
        }
    ...
    {
        public EmptyClauseFilter(ColumnFamilyStore cfs, DataRange range, int maxResults, boolean countCQL3Rows)
        {
            super(cfs, range, maxResults, countCQL3Rows);
        }
