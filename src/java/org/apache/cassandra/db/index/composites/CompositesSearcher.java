            ...
            protected Row computeNext()
            {
                ...
                while (true)
                {
                    ...
                    while (!indexColumns.isEmpty() && columnsCount <= limit)
                    {
                        ...
                        {
                            // We're done with our current row
                        ...
                        // For sparse composites, we're good querying the whole logical row, so we ignore whatever filter was given
                        // Obviously if this index is used for other usage, that might be inefficient
                        CompositeType.Builder builder = baseComparator.builder();
                        for (int i = 0; i < prefixSize; i++)
                            builder.add(components[i + 1]);

                        SliceQueryFilter dataFilter = new SliceQueryFilter(builder.copy().build(), builder.copy().buildAsEndOfRange(), false, Integer.MAX_VALUE);
