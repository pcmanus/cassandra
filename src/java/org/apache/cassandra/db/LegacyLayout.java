...
public abstract class LegacyLayout
{
    ...
    public static UnfilteredRowIterator deserializeLegacyPartition(DataInputPlus in, int version, SerializationHelper.Flag flag, DecoratedKey key) throws IOException
    {
        assert version < MessagingService.VERSION_30;
        ...
        return onWireCellstoUnfilteredRowIterator(metadata, key, info, cells, false, helper);
    }
    ...
    public static class CellGrouper
    {
        ...
        private CellGrouper(CFMetaData metadata, SerializationHelper helper, boolean isStatic)
        {
            ...
            this.builder = ArrayBackedRow.unsortedBuilder(isStatic ? metadata.partitionColumns().statics : metadata.partitionColumns().regulars, FBUtilities.nowInSeconds());
        }
    ...
    public static class LegacyBound
    {
        ...
        {
            assert bound.size() == metadata.comparator.size() : "bound: " + bound.size() + ", comparator=" + metadata.comparator;
