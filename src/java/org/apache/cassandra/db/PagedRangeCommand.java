...
public class PagedRangeCommand extends AbstractRangeCommand
{
    ...
    public PagedRangeCommand(String keyspace,
                             ...
                             int limit)
    {
        super(keyspace, columnFamily, keyRange, predicate, rowFilter);
        this.start = start;
        this.stop = stop;
        this.limit = limit;
    }
    ...
    public List<Row> executeLocally()
    {
        ...
        ExtendedFilter exFilter = cfs.makeExtendedFilter(keyRange, (SliceQueryFilter)predicate, start, stop, rowFilter, limit);
    ...
    {
        public void serialize(PagedRangeCommand cmd, DataOutput dos, int version) throws IOException
        {
            dos.writeUTF(cmd.keyspace);
            dos.writeUTF(cmd.columnFamily);

            AbstractBounds.serializer.serialize(cmd.keyRange, dos, version);

            // SliceQueryFilter (the count is not used)
            SliceQueryFilter filter = (SliceQueryFilter)cmd.predicate;
            SliceQueryFilter.serializer.serialize(filter, dos, version);

            // The start and stop of the page
            ByteBufferUtil.writeWithShortLength(cmd.start, dos);
            ByteBufferUtil.writeWithShortLength(cmd.stop, dos);

            dos.writeInt(cmd.rowFilter.size());
            ...
            dos.writeInt(cmd.limit);
        }

        public PagedRangeCommand deserialize(DataInput dis, int version) throws IOException
        {
            String keyspace = dis.readUTF();
            String columnFamily = dis.readUTF();

            AbstractBounds<RowPosition> keyRange = AbstractBounds.serializer.deserialize(dis, version).toRowBounds();

            SliceQueryFilter predicate = SliceQueryFilter.serializer.deserialize(dis, version);

            ByteBuffer start = ByteBufferUtil.readWithShortLength(dis);
            ByteBuffer stop = ByteBufferUtil.readWithShortLength(dis);

            int filterCount = dis.readInt();
            List<IndexExpression> rowFilter = new ArrayList<IndexExpression>(filterCount);
            for (int i = 0; i < filterCount; i++)
            {
                IndexExpression expr = new IndexExpression(ByteBufferUtil.readWithShortLength(dis),
            ...
            return new PagedRangeCommand(keyspace, columnFamily, keyRange, predicate, start, stop, rowFilter, limit);
        }
