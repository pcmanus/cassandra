...
public class SelectStatement implements CQLStatement
{
        ...
        while (!pager.isExhausted())
        {
            try (PartitionIterator iter = pager.fetchPage(pageSize))
            {
                while (iter.hasNext())
                {
                    ...
                    {
                        processPartition(partition, result, options);
                    }
                }
            }
        }
    ...
    {
        return executeInternal(state, queryOptions, FBUtilities.nowInSeconds());
    }

    public ResultMessage.Rows executeInternal(QueryState state, QueryOptions queryOptions, int nowInSec) throws RequestExecutionException, RequestValidationException
    {
        SelectOptions.Finalized options = selectOptions.finalize(queryOptions, nowInSec);
        ReadQuery query = getQuery(options);
        int pageSize = options.pageSize();
    ...
    void processPartition(RowIterator partition, Selection.ResultSetBuilder result, SelectOptions.Finalized options)
    ...
    private static void addValue(Selection.ResultSetBuilder result, ColumnDefinition def, Row row, SelectOptions.Finalized options)
    {
        if (def.isComplex())
        {
            ...
            {
                Iterator<Cell> cells = options.columnFilter().filterComplexCells(def, complexData.iterator());
                if (def.type.isCollection())
                    result.add(((CollectionType) def.type).serializeForNativeProtocol(cells, result.protocolVersion));
                else
                    result.add(((UserType) def.type).serializeForNativeProtocol(cells, result.protocolVersion));
            }
        }
        else
        {
            result.add(row.getCell(def), options.nowInSec());
        }
    }
