...
public class SelectStatement implements CQLStatement, MeasurableForPreparedCache
{
    ...
    public ResultMessage.Rows execute(QueryState state, QueryOptions options) throws RequestExecutionException, RequestValidationException
    {
        ...
        {
            command = getRangeCommand(options, limit, now);
        }
        else
        {
            List<ReadCommand> commands = getSliceCommands(options, limit, now);
        ...
        else
        {
            ...
            if (parameters.isCount)
                return pageCountQuery(pager, options, pageSize, now);
    ...
    private ResultMessage.Rows pageCountQuery(QueryPager pager, QueryOptions options, int pageSize, long now) throws RequestValidationException, RequestExecutionException
    {
        int count = 0;
    ...
    public ResultMessage.Rows executeInternal(QueryState state) throws RequestExecutionException, RequestValidationException
    {
        ...
        if (isKeyRange || usesSecondaryIndexing)
        {
            RangeSliceCommand command = getRangeCommand(options, limit, now);
    ...
    private ResultSet process(List<Row> rows, QueryOptions options, int limit, long now) throws InvalidRequestException
    {
        ...
        for (org.apache.cassandra.db.Row row : rows)
        {
            ...
            processColumnFamily(row.key.key, row.cf, options, now, result);
        }
