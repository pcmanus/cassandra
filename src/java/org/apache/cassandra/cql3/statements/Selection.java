...
public abstract class Selection
{
    ...
    private static Selector makeSelector(CFDefinition cfDef, RawSelector raw, List<CFDefinition.Name> names, List<ColumnSpecification> metadata) throws InvalidRequestException
    {
        ...
        else if (raw.selectable instanceof Selectable.WithFieldSelection)
        {
            ...
            Selector selected = makeSelector(cfDef, new RawSelector(withField.selected, null), names, null);
            ...
            {
                if (!ut.columnNames.get(i).equals(withField.field.key))
                    continue;

                if (metadata != null)
                    metadata.add(makeFieldSelectSpec(cfDef, withField, ut.types.get(i), raw.alias));
    ...
    private static ColumnSpecification makeFieldSelectSpec(CFDefinition cfDef, Selectable.WithFieldSelection s, AbstractType<?> type, ColumnIdentifier alias)
    {
        return new ColumnSpecification(cfDef.cfm.ksName,
                                       cfDef.cfm.cfName,
