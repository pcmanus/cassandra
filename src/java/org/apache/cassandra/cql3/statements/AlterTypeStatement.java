...
public abstract class AlterTypeStatement extends SchemaAlteringStatement
{
    ...
    {
        UserType toUpdate = Schema.instance.userTypes.getType(name.key);
    ...
    private static int getIdxOfField(UserType type, ColumnIdentifier field)
    {
        for (int i = 0; i < type.types.size(); i++)
            if (field.key.equals(type.columnNames.get(i)))
                return i;
        return -1;
    }
    ...
    {
        AbstractType<?> t = updateWith(def.getValidator(), toReplace, updated);
        if (t == null)
            return false;
    ...
    private static class TypeRename extends AlterTypeStatement
    {
        ...
        protected UserType makeUpdatedType(UserType toUpdate) throws InvalidRequestException
        {
            UserType previous = Schema.instance.userTypes.getType(newName.key);
            if (previous != null)
            ...
            return new UserType(newName.key, new ArrayList<>(toUpdate.columnNames), new ArrayList<>(toUpdate.types));
        }
    }
}
