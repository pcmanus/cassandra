...
public class SSTableIndexWriter implements PerIndexWriter
{
    ...
    private final ComponentGroup.Writer groupWriter;
    ...
    public SSTableIndexWriter(ComponentGroup.Writer groupWriter, NamedMemoryLimiter limiter, BooleanSupplier isIndexValid, long keyCount)
    {
        this.groupWriter = groupWriter;
        this.indexContext = groupWriter.context();
        Preconditions.checkNotNull(indexContext, "Provided group %s is the per-sstable group, expected a per-index one", groupWriter);
        ...
        try
        {
            ...
            if (currentBuilder != null)
            {
                ...
                logger.debug(indexContext.logMessage("Flushing final segment for SSTable {} released {}. Global segment memory usage now at {}."),
                             groupWriter.descriptor(), FBUtilities.prettyPrintMemory(bytesAllocated), FBUtilities.prettyPrintMemory(globalBytesUsed));
            }

            writeSegmentsMetadata();
            groupWriter.markComplete();
        }
        finally
        {
    ...
    public void abort(Throwable cause)
    {
        ...
        logger.warn(indexContext.logMessage("Aborting SSTable index flush for {}..."), groupWriter.descriptor(), cause);
        ...
        if (currentBuilder != null)
        {
            ...
            logger.debug(indexContext.logMessage("Aborting index writer for SSTable {} released {}. Global segment memory usage now at {}."),
                         groupWriter.descriptor(), FBUtilities.prettyPrintMemory(allocated), FBUtilities.prettyPrintMemory(globalBytesUsed));
        }

        if (CassandraRelevantProperties.DELETE_CORRUPT_SAI_COMPONENTS.getBoolean())
            groupWriter.forceDeleteAllComponents();
        else
            logger.debug("Skipping delete of index components after failure on index build of {}.{}", groupWriter.indexDescriptor(), indexContext);
    }
    ...
    private void flushSegment() throws IOException
    {
        ...
        {
            logger.error(indexContext.logMessage("Failed to build index for SSTable {}."), groupWriter.descriptor(), t);
            groupWriter.forceDeleteAllComponents();
    ...
    private void writeSegmentsMetadata() throws IOException
    {
        ...
        try (MetadataWriter writer = new MetadataWriter(groupWriter))
        {
    ...
    private SegmentBuilder newSegmentBuilder(long rowIdOffset)
    {
        ...
        if (indexContext.isVector())
        {
            ...
            if (pqi == null || pqi.unitVectors.isEmpty() || !V3OnDiskFormat.ENABLE_LTM_CONSTRUCTION) {
                builder = new SegmentBuilder.VectorOnHeapSegmentBuilder(groupWriter, rowIdOffset, keyCount, limiter);
            } else {
                builder = new SegmentBuilder.VectorOffHeapSegmentBuilder(groupWriter, rowIdOffset, keyCount, limiter, pqi.pq, pqi.unitVectors.get());
            }
        }
        else if (indexContext.isLiteral())
        {
            builder = new SegmentBuilder.RAMStringSegmentBuilder(groupWriter, rowIdOffset, limiter);
        }
        else
        {
            builder = new SegmentBuilder.KDTreeSegmentBuilder(groupWriter, rowIdOffset, limiter, indexContext.getIndexWriterConfig());
        }
