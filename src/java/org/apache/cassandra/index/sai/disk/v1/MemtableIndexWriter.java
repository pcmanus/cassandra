...
public class MemtableIndexWriter implements PerIndexWriter
{
    ...
    public MemtableIndexWriter(MemtableIndex memtableIndex,
                               ComponentGroup.Writer groupWriter,
    ...
    {
        logger.warn(groupWriter.logMessage("Aborting index memtable flush for {}..."), groupWriter.descriptor(), cause);
    ...
    public void complete(Stopwatch stopwatch) throws IOException
    {
        ...
        try
        {
            ...
            {
                logger.debug(groupWriter.logMessage("No indexed rows to flush from SSTable {}."), groupWriter.descriptor());
        ...
        {
            logger.error(groupWriter.logMessage("Error while flushing index {}"), t.getMessage(), t);
    ...
    {
        return groupWriter.indexDescriptor().primaryKeyFactory;
    }
    ...
    private long flush(DecoratedKey minKey, DecoratedKey maxKey, AbstractType<?> termComparator, MemtableTermsIterator terms, long maxSegmentRowId) throws IOException
    {
        ...
        {
            try (InvertedIndexWriter writer = new InvertedIndexWriter(groupWriter))
            {
        ...
        {
            try (NumericIndexWriter writer = new NumericIndexWriter(groupWriter,
    ...
    private void flushVectorIndex(DecoratedKey minKey, DecoratedKey maxKey, long startTime, Stopwatch stopwatch) throws IOException
    {
        ...
        {
            logger.debug(groupWriter.logMessage("Whole graph is deleted. Skipping index flush for {}."), groupWriter.descriptor());
            groupWriter.markComplete();
            return;
        }

        SegmentMetadata.ComponentMetadataMap metadataMap = vectorIndex.writeData(groupWriter, deletedOrdinals);
    ...
    private void completeIndexFlush(long cellCount, long startTime, Stopwatch stopwatch) throws IOException
    {
        ...
        logger.debug(groupWriter.logMessage("Completed flushing {} memtable index cells to SSTable {}. Duration: {} ms. Total elapsed: {} ms"),
                     cellCount,
