...
public abstract class ColumnIndexSearcher
{
    private static final Logger logger = LoggerFactory.getLogger(ColumnIndexSearcher.class);

    private final ColumnIndexMetadata metadata;
    private final RowFilter.Expression expression;
    protected final CassandraIndex indexer;

    public ColumnIndexSearcher(ColumnIndexMetadata metadata,
    ...
    public UnfilteredPartitionIterator search(ReadCommand command, ReadOrderGroup orderGroup)
    {
        ...
        DecoratedKey indexKey = metadata.indexCfs.decorateKey(expression.getIndexValue());
        UnfilteredRowIterator indexIter = queryIndex(indexKey, command, orderGroup);
        try
        {
            return queryDataFromIndex(metadata, indexKey, UnfilteredRowIterators.filter(indexIter, command.nowInSec()), command, orderGroup);
        }
    ...
    private UnfilteredRowIterator queryIndex(DecoratedKey indexKey, ReadCommand command, ReadOrderGroup orderGroup)
    {
        ...
        CFMetaData indexCfm = metadata.indexCfs.metadata;
        return SinglePartitionReadCommand.create(indexCfm, command.nowInSec(), indexKey, ColumnFilter.all(indexCfm), filter)
                                         .queryMemtableAndDisk(metadata.indexCfs, orderGroup.indexReadOpOrderGroup());
    }
    ...
    private ClusteringIndexFilter makeIndexFilter(ReadCommand command)
    {
        if (command instanceof SinglePartitionReadCommand)
        {
            ...
            if (filter instanceof ClusteringIndexNamesFilter)
            {
                ...
                BTreeSet.Builder<Clustering> clusterings = BTreeSet.builder(metadata.getIndexComparator());
            ...
            else
            {
                ...
                Slices.Builder builder = new Slices.Builder(metadata.getIndexComparator());
        ...
        else
        {
            ...
            return new ClusteringIndexSliceFilter(Slices.with(metadata.getIndexComparator(), slice), false);
        }
    }

    private Slice.Bound makeIndexBound(ByteBuffer rowKey, Slice.Bound bound)
    {
        return metadata.functions.buildIndexClusteringPrefix(metadata, rowKey, bound, null)
    ...
    {
        return metadata.functions.buildIndexClusteringPrefix(metadata, rowKey, clustering, null).build();
    }

    protected abstract UnfilteredPartitionIterator queryDataFromIndex(ColumnIndexMetadata metadata,
