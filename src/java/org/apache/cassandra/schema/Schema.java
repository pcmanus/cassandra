...
public final class Schema
{
    ...
    public void load(KeyspaceMetadata ksm)
    {
        ...
        if (previous.isPresent())
        {
            ...
            tablesDiff.entriesOnlyOnRight().values().forEach(table -> metadataRefs.put(table.id, TableMetadataRef.to(table)));
            viewsDiff.entriesOnlyOnRight().values().forEach(view -> metadataRefs.put(view.metadata.id, TableMetadataRef.to(view.metadata)));
        ...
        {
            ksm.tablesAndViews().forEach(metadata -> metadataRefs.put(metadata.id, TableMetadataRef.to(metadata)));
        }
    ...
    private void createKeyspace(KeyspaceMetadata keyspace)
    {
        ...
        keyspace.tables.forEach(this::createTable);
        keyspace.views.forEach(this::createView);
        ks.viewManager.reload();
    ...
    {
        ColumnFamilyStore cfs = Keyspace.open(metadata.keyspace).getColumnFamilyStore(metadata.table);
        assert cfs != null;
    ...
    {
        Keyspace.open(view.ksName).initCf(metadataRefs.get(view.metadata.id), true);
    }

    private void alterTable(TableMetadata updated)
    {
        Keyspace.open(updated.keyspace).getColumnFamilyStore(updated.table).reload();
    }

    private void alterView(ViewMetadata updated)
    {
        Keyspace.open(updated.ksName).getColumnFamilyStore(updated.viewName).reload();
    }
    ...
    {
        changeListeners.forEach(l -> l.onCreateTable(metadata.keyspace, metadata.table));
    }

    private void notifyCreateView(ViewMetadata view)
    {
        changeListeners.forEach(l -> l.onCreateView(view.ksName, view.viewName));
    }
    ...
    private void notifyAlterTable(TableMetadata current, TableMetadata updated)
    {
        ...
        changeListeners.forEach(l -> l.onAlterTable(updated.keyspace, updated.table, changeAffectedPreparedStatements));
    }
    ...
    private void notifyAlterView(ViewMetadata current, ViewMetadata updated)
    {
        ...
        changeListeners.forEach(l ->l.onAlterView(updated.ksName, updated.viewName, changeAffectedPreparedStatements));
    }
    ...
    {
        changeListeners.forEach(l -> l.onDropTable(metadata.keyspace, metadata.table));
    }

    private void notifyDropView(ViewMetadata view)
    {
        changeListeners.forEach(l -> l.onDropView(view.ksName, view.viewName));
    }
