...
public class MigrationManager
{
    ...
    private static void announceNewTable(TableMetadata cfm, boolean announceLocally, boolean throwOnDuplicate)
    {
        ...
        if (ksm == null)
            throw new ConfigurationException(String.format("Cannot add table '%s' to non existing keyspace '%s'.", cfm.table, cfm.keyspace));
        // If we have a table or a view which has the same name, we can't add a new one
        else if (throwOnDuplicate && ksm.getTableOrViewNullable(cfm.table) != null)
            throw new AlreadyExistsException(cfm.keyspace, cfm.table);
    ...
    public static void announceNewView(ViewMetadata view, boolean announceLocally) throws ConfigurationException
    {
        ...
        KeyspaceMetadata ksm = Schema.instance.getKeyspaceMetadata(view.ksName);
        if (ksm == null)
    ...
    public static void announceTableUpdate(TableMetadata updated, boolean announceLocally) throws ConfigurationException
    {
        ...
        TableMetadata current = Schema.instance.getTableMetadata(updated.keyspace, updated.table);
        if (current == null)
            throw new ConfigurationException(String.format("Cannot update non existing table '%s' in keyspace '%s'.", updated.table, updated.keyspace));
        KeyspaceMetadata ksm = Schema.instance.getKeyspaceMetadata(current.keyspace);

        current.validateCompatibility(updated);

        logger.info("Update table '{}/{}' From {} To {}", current.keyspace, current.table, current, updated);
    ...
    public static void announceViewUpdate(ViewMetadata view, boolean announceLocally) throws ConfigurationException
    {
        ...
        ViewMetadata oldView = Schema.instance.getView(view.ksName, view.viewName);
        if (oldView == null)
    ...
    public static void announceTableDrop(String ksName, String cfName, boolean announceLocally) throws ConfigurationException
    {
        ...
        logger.info("Drop table '{}/{}'", tm.keyspace, tm.table);
