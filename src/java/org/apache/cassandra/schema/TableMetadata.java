...
public final class TableMetadata
{
    ...
    private final PartitionColumns regularAndStaticColumns;
    ...
    private TableMetadata(Builder builder)
    {
        ...
        regularAndStaticColumns = PartitionColumns.builder().addAll(builder.regularAndStaticColumns).build();
    ...
    public PartitionColumns regularAndStaticColumns()
    {
    ...
    public void validate()
    {
        ...
        if (!isNameValid(table))
            except("Table name must not be empty, more than %s characters long, or contain non-alphanumeric-underscore characters (got \"%s\")", SchemaConstants.NAME_LENGTH, table);
    ...
    void validateCompatibility(TableMetadata other)
    {
        ...
        if (!other.table.equals(table))
            except("Table mismatch (found %s; expected %s)", other.table, table);
        ...
        if (!other.comparator.isCompatibleWith(comparator))
            except("Table comparators do not match or are not compatible (found %s; expected %s).", other.comparator, comparator);
    }
    ...
    public String indexTableName(IndexMetadata info)
    {
        ...
        return table + Directories.SECONDARY_INDEX_NAME_SEPARATOR + info.name;
    }
    ...
    public int hashCode()
    {
        return Objects.hash(keyspace, table, id, partitioner, params, flags, isView, columns, droppedColumns, indexes, triggers);
    }
    ...
    public static final class Builder
    {
        ...
        private Builder(String keyspace, String table)
        {
            this.keyspace = keyspace;
            this.table = table;
        }
        ...
        {
            return addColumn(new ColumnMetadata(keyspace, table, name, type, partitionKeyColumns.size(), ColumnMetadata.Kind.PARTITION_KEY));
        }
        ...
        {
            return addColumn(new ColumnMetadata(keyspace, table, name, type, clusteringColumns.size(), ColumnMetadata.Kind.CLUSTERING));
        }
        ...
        {
            return addColumn(new ColumnMetadata(keyspace, table, name, type, ColumnMetadata.NO_POSITION, ColumnMetadata.Kind.REGULAR));
        }
        ...
        {
            return addColumn(new ColumnMetadata(keyspace, table, name, type, ColumnMetadata.NO_POSITION, ColumnMetadata.Kind.STATIC));
        }
        ...
        public Builder recordDeprecatedSystemColumn(String name, AbstractType<?> type)
        {
            ...
            recordColumnDrop(ColumnMetadata.regularColumn(keyspace, table, name, type), Long.MAX_VALUE);
            return this;
        }
